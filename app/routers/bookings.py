...
def create_booking(
    ...
    with _booking_write_lock:
        room = db.query(Room).filter(Room.id == payload.room_id, Room.org_id == user.org_id).with_for_update().first()
        if room is None:
...
def cancel_booking(
    ...
    with _booking_write_lock:
        ...
        exact_amount = Decimal(booking.price_cents) * (Decimal(refund_percent) / Decimal(100))
        refund_amount_cents = int(exact_amount.quantize(Decimal('1'), rounding=ROUND_HALF_UP))
